...
func fstackPlaybook(lC LaunchContext, rC *runtimeContext, st model.Stack, ust component.UsableComponent, sCs *StepResults) {
	cm := lC.Ekara().ComponentManager()
	sc := InitPlaybookStepResult("Running the stack playbook installation phase", st, NoCleanUpRequired)
...
func fstackCompose(lC LaunchContext, rC *runtimeContext, parent model.Parent, s model.Stack, sCs *StepResults) {
	...
	sc := InitPlaybookStepResult("Running the stack compose installation phase", s, NoCleanUpRequired)

	// Stack install exchange folder for the given provider
	fName := fmt.Sprintf("install_stack_%s", s.Name)
